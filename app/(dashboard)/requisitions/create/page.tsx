...
export default function CreateRequisitionPage() {
  ...
  const fetchInventory = async () => {
    try {
      ...
      if (result.success) {
        setInventory(
          result.data.filter((item: InventoryItem) => item.currentStock > 0)
        );
      }
    } catch (error) {
  ...
  const submitRequisition = async () => {
    ...
    const invalidItems = items.filter(
      (item) => item.item && item.quantity > item.item.currentStock
    );
    ...
    <div className="space-y-6">
      ...
      >
        <div className="p-6 pt-4 space-y-6">
          ...
          {/* Table with Perfect Sticky Header */}
          <div className="rounded-xl border overflow-hidden shadow-sm">
            <div
              className="max-h-96 overflow-y-auto scrollbar-thin scrollbar-thumb-rounded scrollbar-thumb-gray-400 dark:scrollbar-thumb-gray-600"
              style={{
                backgroundColor: "var(--color-surface)",
                borderColor: "var(--color-border)",
              }}
